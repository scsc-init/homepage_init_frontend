...
export default function PigLeaderChangeButton({ pigId, canChange, isOwner, members, owner }) {
...
const changeBySelf = async (newowner) => {
  const jwt = ensureJwt();
...
const changeByExec = async (newowner) => {
  const jwt = ensureJwt();
  newOwner = newowner.id;
  if (!jwt) return;
  try {
