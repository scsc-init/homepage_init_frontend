...
export default async function handler(req, res) {
  ...
  const { frontend_secret, email, name, phone, student_id, major_id } =
    req.body;
  ...
  try {
    ...
    const createdUser = await db.get("SELECT * FROM user WHERE id = ?", [
      result.lastID,
    ]);
    ...
    if (err.message.includes("UNIQUE constraint failed")) {
      return res
        .status(409)
        .json({ error: "Duplicate email, phone, or student_id" });
    }
    console.error(err);
